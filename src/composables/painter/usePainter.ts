...
export function usePainter(nodeId: NodeId, options: UsePainterOptions) {
  ...
  function painterWidgetId(name: string): WidgetId | null {
  ...
  function widgetValue(name: string): unknown {
    const id = painterWidgetId(name)
    return id ? widgetValueStore.getWidget(id)?.value : undefined
  }

  function setWidgetValue(name: string, value: number | string): void {
    const id = painterWidgetId(name)
    if (id) widgetValueStore.setValue(id, value)
  }
  ...
  const backgroundColor = computed<string>({
    get: () => {
      const v = widgetValue('bg_color')
      return typeof v === 'string' ? v : '#000000'
    },
    set: (v) => setWidgetValue('bg_color', v)
  })
